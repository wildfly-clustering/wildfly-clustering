...
public class HotRodSessionManagerITCase extends AbstractSessionManagerITCase<HotRodSessionManagerParameters> {
	...
	static class HotRodSessionManagerArgumentsProvider implements ArgumentsProvider {
		...
		Set<TransactionMode> transactionModes = EnumSet.of(TransactionMode.NONE, TransactionMode.NON_DURABLE_XA);
					...
					for (SessionAttributePersistenceStrategy strategy : EnumSet.allOf(SessionAttributePersistenceStrategy.class)) {
						builder.add(Arguments.of(new HotRodSessionManagerParameters() {
							...
							public String toString() {
								return Map.of(ByteBufferMarshaller.class.getSimpleName(), marshaller.toString(), SessionAttributePersistenceStrategy.class.getSimpleName(), strategy, TransactionMode.class.getSimpleName(), transactionMode).toString();
							}
						}));
					}
				}
	...
	public void concurrent(HotRodSessionManagerParameters parameters) throws InterruptedException {
