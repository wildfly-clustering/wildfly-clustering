...
public class FastConcurrentDirectDeque<E> extends AbstractCollection<E> implements ConcurrentDirectDeque<E>, Serializable {
	 ...
	 * <code>p.prev == null && p.next != p</code>
	...
	private void initHeadTail(Node<E> head, Node<E> tail) {
		this.head = (head != null) ? head : new Node<>();
		if (head == tail) {
			if (head == null) {
				this.tail = this.head;
			} else {
				...
				NEXT.set(tail, newNode);
				PREV.set(newNode, tail);
				this.tail = newNode;
			}
		} else {
			this.tail = tail;
	...
	static {
		...
		try {
			...
			HEAD = l.findVarHandle(FastConcurrentDirectDeque.class, "head", Node.class);
			TAIL = l.findVarHandle(FastConcurrentDirectDeque.class, "tail", Node.class);
